...
if [ -e "config.csv" ]; then
    ...
    VEGETATION_DATA=$(grep ^VEGETATION_DATA, "config.csv" | cut -f2- -d, | tr ',' ' ')
...
if [ "$ANALYSIS" = "vegetation_analysis" ]; then 
    ...
    python3 /usr/local/share/gridlabd/template/US/CA/SLAC/anticipation/testing_geopandas.py
    ...
    python3 /usr/local/share/gridlabd/template/US/CA/SLAC/anticipation/critical_load.py 123.json 
